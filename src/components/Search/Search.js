...
function Search({ store, dispatch }) {
  ...
  const example = () => { alert('É só um exemplo de interface ainda está em desenvolvimento'); };
  ...
  return (
    <section data-testid="app-search" className="App-search">
      <label className="label-search">
        ...
        <input className="App-search-input" type="text" placeholder="Pesquise no Google ou digite um URL" value={store.search} onChange={e => dispatch({type: 'SEARCH_VALUE', payload: e.target.value})} onKeyPress={searchEnter}/>
      </label>
      <button type="button" className={ `btn clean${store.search ? ' active' : '' }`} onClick={searchClean}>
