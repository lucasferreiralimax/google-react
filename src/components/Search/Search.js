...
function Search({ store, dispatch }, props) {
  ...
  return (
    <section data-testid="app-search" className="App-search">
      ...
      <input className="App-search-input" type="text" placeholder="Pesquise no Google ou digite um URL" value={name} onChange={e => setName(e.target.value)} onKeyPress={searchEnter}/>
      <button type="button" className={ `btn clean${name ? ' active' : '' }`} onClick={searchClean}>
