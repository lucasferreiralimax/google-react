  ...
  return (
    <section data-testid="app-profile" className="App-profile" ref={wrapperRef}>
      ...
      <div className={ `App-profile__content${show ? ' show' : '' }`}>
        <button className="btn-close" aria-label="Close menu" onClick={() => setShow(false)}>
          <svg width="22" height="22" viewBox="1 1 22 22" focusable="false" fill="var(--main-color)">
            <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12 19 6.41z" />
          </svg>
        </button>
        <p className="email">lucasferreiralimax@gmail.com</p>
        ...
        <p className="title">Lucas Lima</p>
        <button className="btn" type="button">{t('profile_component.config')}</button>
        <div className="divider">
          <a className="link" href="https://github.com/lucasferreiralimax" target="_blank" rel="noreferrer noopener">{t('profile_component.politics')}</a>
          <div className="bullet">•</div>
          <a className="link" href="https://github.com/lucasferreiralimax" target="_blank" rel="noreferrer noopener">{t('profile_component.terms')}</a>
        </div>
      </div>
    </section>
  );
